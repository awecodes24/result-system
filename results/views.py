...
def student_list(request):
    ...
    if query:
        students = students.filter(full_name_icontains=query) | \
            students.filter(roll_numbericontains=query)
    if semester:
        students = students.filter(semester=semester)
    return render(request, 'results/student_list.html', 
                  {'students': students, 'query': query, 'semester': semester})
...
def edit_student(request, pk):
    ...
    if form.is_valid():
        ...
        return redirect('student_list')
    return render(request, 'results/edit_student.html', {'form':form, 'title':'Edit Student'})
...
def delete_student(request, pk):
    ...
    if request.method == 'POST':
        ...
        return redirect('student_list')
    return render(request, 'results/confirm_delete.html', {'object': student})
...
def add_result(request, pk):
    ...
    if form.is_valid():
        ...
        return redirect('student_report', pk=form.instance.student.pk)
    return render(request, 'results/add_result.html', {'forms': form})
...
def search_result(request):
    ...
    return render(request, 'results/search.html', {'form': form, 'student': student, 'results': results})
